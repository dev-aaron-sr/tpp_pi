from datetime import datetime
from decimal import Decimal, InvalidOperation
from typing import Dict, List, Optional
import uuid
import customtkinter as ctk


class PesajeView(ctk.CTkFrame):

  def __init__(
      self,
      parent,
      agricultor_dao,
      recepcion_dao,
      sesion_dao,
      scale_service,
      print_service,
      sesion_activa,
      on_cerrar_sesion_cb,
  ):
    super().__init__(parent, fg_color="#CBD5E1", corner_radius=0)

    self.agricultor_dao = agricultor_dao
    self.recepcion_dao = recepcion_dao
    self.sesion_dao = sesion_dao
    self.scale_service = scale_service
    self.print_service = print_service
    self.sesion_activa = sesion_activa
    self.on_cerrar_sesion_cb = on_cerrar_sesion_cb

    # Estado local
    self.agricultor_actual: Optional[Dict] = None
    self.recepcion_uuid_actual: Optional[str] = None
    self.destino_actual = "MERCADO"
    self.pesadas = {"MERCADO": [], "FABRICA_LOCAL": [], "FABRICA_PLANTA": []}
    self.filas_ui = {"MERCADO": {}, "FABRICA_LOCAL": {}, "FABRICA_PLANTA": {}}
    self.parcelas_map = {}

    self._build_ui()
    self._bind_shortcuts()
    self._iniciar_lectura_balanza()

  def _build_ui(self):
    # ---------------------------------------------------------------------
    # BARRA SUPERIOR INFORMATIVA DE SESIÓN
    # ---------------------------------------------------------------------
    self.top_session_bar = ctk.CTkFrame(
        self, fg_color="#1E293B", height=40, corner_radius=0
    )
    self.top_session_bar.pack(fill="x", side="top")
    self.top_session_bar.pack_propagate(False)

    cod_sesion = self.sesion_activa.get("codigo_sesion", "S/N")
    f_apertura = self.sesion_activa.get("fecha_apertura", "")

    ctk.CTkLabel(
        self.top_session_bar,
        text=f"🟢 SESIÓN ACTIVA: {cod_sesion} | Inicio: {f_apertura}",
        font=("Segoe UI", 12, "bold"),
        text_color="#4ADE80",
    ).pack(side="left", padx=15)

    ctk.CTkButton(
        self.top_session_bar,
        text="🔒 CERRAR JORNADA",
        font=("Segoe UI", 11, "bold"),
        fg_color="#DC2626",
        hover_color="#B91C1C",
        width=130,
        height=28,
        command=self._ejecutar_cierre_jornada,
    ).pack(side="right", padx=15)

    # ---------------------------------------------------------------------
    # PANEL IZQUIERDO: AGRICULTOR Y TOTALES
    # ---------------------------------------------------------------------
    self.panel_left = ctk.CTkFrame(
        self,
        fg_color="#E2E8F0",
        border_width=2,
        border_color="#64748B",
        width=380,
        corner_radius=4,
    )
    self.panel_left.pack(side="left", fill="y", padx=6, pady=6)
    self.panel_left.pack_propagate(False)

    ctk.CTkLabel(
        self.panel_left,
        text="Código Padrón",
        font=("Arial", 20, "bold"),
        text_color="#0F172A",
    ).pack(anchor="w", padx=14, pady=(12, 2))

    self.txt_padron = ctk.CTkEntry(
        self.panel_left,
        font=("Arial", 22, "bold"),
        fg_color="#FFFFFF",
        text_color="#000000",
        border_color="#334155",
        border_width=2,
        corner_radius=2,
        height=46,
    )
    self.txt_padron.pack(fill="x", padx=14, pady=(0, 8))

    ctk.CTkLabel(
        self.panel_left,
        text="Socio:",
        font=("Arial", 18, "bold"),
        text_color="#0F172A",
    ).pack(anchor="w", padx=14)
    self.lbl_socio_nombre = ctk.CTkLabel(
        self.panel_left,
        text="[ingrese un codigo padron]",
        font=("Arial", 18, "bold"),
        text_color="#334155",
        wraplength=340,
        justify="left",
    )
    self.lbl_socio_nombre.pack(anchor="w", padx=14, pady=(0, 4))

    ctk.CTkLabel(
        self.panel_left,
        text="DNI/RUC:",
        font=("Arial", 18, "bold"),
        text_color="#0F172A",
    ).pack(anchor="w", padx=14)
    self.lbl_socio_doc = ctk.CTkLabel(
        self.panel_left, text="-", font=("Arial", 18, "bold"), text_color="#0F172A"
    )
    self.lbl_socio_doc.pack(anchor="w", padx=14, pady=(0, 6))

    ctk.CTkLabel(
        self.panel_left,
        text="Parcela:",
        font=("Arial", 18, "bold"),
        text_color="#0F172A",
    ).pack(anchor="w", padx=14)
    self.cmb_parcela = ctk.CTkOptionMenu(
        self.panel_left,
        values=["---"],
        fg_color="#FFFFFF",
        text_color="#000000",
        button_color="#1E3A8A",
        dropdown_fg_color="#FFFFFF",
        dropdown_text_color="#000000",
        corner_radius=2,
        height=40,
        font=("Arial", 18, "bold"),
    )
    self.cmb_parcela.pack(fill="x", padx=14, pady=(0, 8))

    ctk.CTkFrame(
        self.panel_left, height=2, fg_color="#64748B", corner_radius=0
    ).pack(fill="x", padx=6, pady=6)

    # TOTALES ACUMULADOS
    ctk.CTkLabel(
        self.panel_left,
        text="TOTALES",
        font=("Arial", 18, "bold"),
        text_color="#0F172A",
    ).pack(anchor="w", padx=14, pady=2)

    frame_tot_m = ctk.CTkFrame(self.panel_left, fg_color="transparent")
    frame_tot_m.pack(fill="x", padx=14, pady=1)
    ctk.CTkLabel(
        frame_tot_m,
        text="Mercado:",
        font=("Arial", 16, "bold"),
        text_color="#0F172A",
    ).pack(side="left")
    self.lbl_tot_mercado = ctk.CTkLabel(
        frame_tot_m,
        text="0.00kg",
        font=("Arial", 18, "bold"),
        text_color="#0284C7",
    )
    self.lbl_tot_mercado.pack(side="right")

    frame_tot_fl = ctk.CTkFrame(self.panel_left, fg_color="transparent")
    frame_tot_fl.pack(fill="x", padx=14, pady=1)
    ctk.CTkLabel(
        frame_tot_fl,
        text="Fab. Local:",
        font=("Arial", 16, "bold"),
        text_color="#0F172A",
    ).pack(side="left")
    self.lbl_tot_fab_local = ctk.CTkLabel(
        frame_tot_fl,
        text="0.00kg",
        font=("Arial", 18, "bold"),
        text_color="#D97706",
    )
    self.lbl_tot_fab_local.pack(side="right")

    frame_tot_fp = ctk.CTkFrame(self.panel_left, fg_color="transparent")
    frame_tot_fp.pack(fill="x", padx=14, pady=1)
    ctk.CTkLabel(
        frame_tot_fp,
        text="Fab. Planta:",
        font=("Arial", 16, "bold"),
        text_color="#0F172A",
    ).pack(side="left")
    self.lbl_tot_fab_planta = ctk.CTkLabel(
        frame_tot_fp,
        text="0.00kg",
        font=("Arial", 18, "bold"),
        text_color="#7C3AED",
    )
    self.lbl_tot_fab_planta.pack(side="right")

    # CONTENEDOR DE ACCIONES (PAUSAR Y FINALIZAR)
    frame_acciones = ctk.CTkFrame(self.panel_left, fg_color="transparent")
    frame_acciones.pack(fill="x", side="bottom", padx=14, pady=12)

    self.btn_pausar = ctk.CTkButton(
        frame_acciones,
        text="⏸️ PAUSAR / OTRO VIAJE",
        font=("Arial", 16, "bold"),
        fg_color="#334155",
        hover_color="#1E293B",
        text_color="#FFFFFF",
        corner_radius=2,
        height=48,
        command=self._pausar_pesaje,
    )
    self.btn_pausar.pack(fill="x", pady=(0, 8))

    self.btn_finalizar = ctk.CTkButton(
        frame_acciones,
        text="✅ FINALIZAR Y EMITIR RECIBO",
        font=("Arial", 18, "bold"),
        fg_color="#16A34A",
        hover_color="#15803D",
        text_color="#FFFFFF",
        corner_radius=2,
        height=58,
        command=self._finalizar_pesaje_socio,
    )
    self.btn_finalizar.pack(fill="x")

    # ---------------------------------------------------------------------
    # PANEL CENTRAL: BOTONES DE DESTINO, VISOR LCD Y TABLAS
    # ---------------------------------------------------------------------
    self.panel_center = ctk.CTkFrame(self, fg_color="transparent")
    self.panel_center.pack(
        side="left", fill="both", expand=True, padx=4, pady=6
    )

    self.frame_tabs = ctk.CTkFrame(
        self.panel_center, fg_color="transparent", height=42
    )
    self.frame_tabs.pack(fill="x", side="top", pady=(0, 6))

    self.btn_f1 = ctk.CTkButton(
        self.frame_tabs,
        text="MERCADO [F1]",
        font=("Arial", 16, "bold"),
        fg_color="#0284C7",
        hover_color="#0369A1",
        text_color="#FFFFFF",
        corner_radius=2,
        width=150,
        height=42,
        command=lambda: self._set_destino("MERCADO"),
    )
    self.btn_f1.pack(side="left", padx=(0, 4))

    self.btn_f2 = ctk.CTkButton(
        self.frame_tabs,
        text="FAB. LOCAL [F2]",
        font=("Arial", 16, "bold"),
        fg_color="#E2E8F0",
        hover_color="#D97706",
        text_color="#000000",
        corner_radius=2,
        width=150,
        height=42,
        command=lambda: self._set_destino("FABRICA_LOCAL"),
    )
    self.btn_f2.pack(side="left", padx=4)

    self.btn_f3 = ctk.CTkButton(
        self.frame_tabs,
        text="FAB. PLANTA [F3]",
        font=("Arial", 16, "bold"),
        fg_color="#E2E8F0",
        hover_color="#6D28D9",
        text_color="#000000",
        corner_radius=2,
        width=150,
        height=42,
        command=lambda: self._set_destino("FABRICA_PLANTA"),
    )
    self.btn_f3.pack(side="left", padx=4)

    # Visor LCD Gigante
    self.frame_visor = ctk.CTkFrame(
        self.panel_center,
        fg_color="#000000",
        height=180,
        corner_radius=2,
        border_width=2,
        border_color="#475569",
    )
    self.frame_visor.pack(fill="x", side="top", pady=(0, 8))
    self.frame_visor.pack_propagate(False)

    self.lbl_unit = ctk.CTkLabel(
        self.frame_visor, text="kg", font=("Arial", 40, "bold"), text_color="#00FF00"
    )
    self.lbl_unit.pack(side="right", padx=(0, 20), pady=(40, 0))

    self.lbl_peso = ctk.CTkLabel(
        self.frame_visor,
        text="0.00",
        font=("Consolas", 180, "bold"),
        text_color="#00FF00",
    )
    self.lbl_peso.pack(side="right", padx=(0, 10))

    self.frame_bottom_grid = ctk.CTkFrame(
        self.panel_center, fg_color="transparent"
    )
    self.frame_bottom_grid.pack(fill="both", expand=True)

    self.frame_tables = ctk.CTkFrame(
        self.frame_bottom_grid, fg_color="transparent"
    )
    self.frame_tables.pack(side="left", fill="both", expand=True, padx=(0, 6))

    self.panel_tabla_mercado = self._crear_estructura_tabla(
        "MERCADO", self.frame_tables, header_color="#0284C7"
    )
    self.panel_tabla_fab_local = self._crear_estructura_tabla(
        "FABRICA LOCAL", self.frame_tables, header_color="#D97706"
    )
    self.panel_tabla_fab_planta = self._crear_estructura_tabla(
        "FABRICA PLANTA", self.frame_tables, header_color="#7C3AED"
    )

    self.panel_right = ctk.CTkFrame(
        self.frame_bottom_grid,
        fg_color="#E2E8F0",
        border_width=2,
        border_color="#64748B",
        width=200,
        corner_radius=2,
    )
    self.panel_right.pack(side="right", fill="y")
    self.panel_right.pack_propagate(False)

    ctk.CTkLabel(
        self.panel_right,
        text="cant de sacos",
        font=("Arial", 20, "bold"),
        text_color="#000000",
    ).pack(pady=(12, 4))
    self.txt_sacos = ctk.CTkEntry(
        self.panel_right,
        font=("Arial", 30, "bold"),
        justify="center",
        fg_color="#FFFFFF",
        text_color="#000000",
        border_color="#000000",
        border_width=2,
        corner_radius=2,
        height=48,
    )
    self.txt_sacos.pack(fill="x", padx=10, pady=(0, 8))
    self.txt_sacos.insert(0, "1")

    # ---------------------------------------------------------------------
    # CHECKBOX DE IMPRESIÓN DE ETIQUETA (SALDOS DE LOTE)
    # ---------------------------------------------------------------------
    self.chk_imprimir_etiqueta = ctk.CTkCheckBox(
        self.panel_right,
        text="Imprimir Etiqueta",
        font=("Arial", 13, "bold"),
        text_color="#0F172A",
        fg_color="#0284C7",
        hover_color="#0369A1",
        corner_radius=2,
        checkbox_width=22,
        checkbox_height=22,
    )
    self.chk_imprimir_etiqueta.pack(fill="x", padx=10, pady=(0, 10))
    self.chk_imprimir_etiqueta.select()  # Activado por defecto

    self.btn_registrar = ctk.CTkButton(
        self.panel_right,
        text="REGISTRAR\n[ENTER]",
        font=("Arial", 20, "bold"),
        fg_color="#0284C7",
        hover_color="#0369A1",
        text_color="#FFFFFF",
        corner_radius=2,
        height=60,
        command=self._registrar_bajada,
    )
    self.btn_registrar.pack(fill="x", padx=10, pady=5)

    self.btn_eliminar = ctk.CTkButton(
        self.panel_right,
        text="ELIMINAR\nULTIMO",
        font=("Arial", 20, "bold"),
        fg_color="#EF4444",
        hover_color="#DC2626",
        text_color="#FFFFFF",
        corner_radius=2,
        height=52,
        command=self._eliminar_ultimo,
    )
    self.btn_eliminar.pack(fill="x", padx=10, pady=5)

    self.btn_reiniciar = ctk.CTkButton(
        self.panel_right,
        text="REINICIAR\nPESAJE",
        font=("Arial", 20, "bold"),
        fg_color="#991B1B",
        hover_color="#7F1D1D",
        text_color="#FFFFFF",
        corner_radius=2,
        height=58,
        command=self._reiniciar_pesaje,
    )
    self.btn_reiniciar.pack(fill="x", side="bottom", padx=10, pady=10)

  def _crear_estructura_tabla(
      self, nombre_destino: str, contenedor_padre, header_color: str
  ) -> Dict:
    box = ctk.CTkFrame(
        contenedor_padre,
        fg_color="#FFFFFF",
        border_width=2,
        border_color="#000000",
        corner_radius=0,
    )
    box.pack(side="left", fill="both", expand=True, padx=2)

    lbl_header = ctk.CTkLabel(
        box,
        text=nombre_destino,
        font=("Arial", 20, "bold"),
        fg_color=header_color,
        text_color="#FFFFFF",
        height=38,
        corner_radius=0,
    )
    lbl_header.pack(fill="x", side="top")

    sub_head = ctk.CTkFrame(
        box,
        fg_color="#E2E8F0",
        height=45,
        corner_radius=0,
        border_width=1,
        border_color="#000000",
    )
    sub_head.pack(fill="x", side="top")

    ctk.CTkLabel(
        sub_head,
        text="Peso",
        font=("Arial", 20, "bold"),
        text_color="#000000",
        width=140,
    ).pack(side="left", padx=5, pady=5)
    ctk.CTkLabel(
        sub_head,
        text="Sacos",
        font=("Arial", 20, "bold"),
        text_color="#000000",
        width=80,
    ).pack(side="left", padx=5)
    ctk.CTkLabel(sub_head, text="", width=35).pack(side="right", padx=3)

    scroll = ctk.CTkScrollableFrame(box, fg_color="#FFFFFF", corner_radius=0)
    scroll.pack(fill="both", expand=True)

    footer = ctk.CTkFrame(
        box,
        fg_color="#FFFFFF",
        height=42,
        corner_radius=0,
        border_width=1,
        border_color="#000000",
    )
    footer.pack(fill="x", side="bottom")

    lbl_tot_peso = ctk.CTkLabel(
        footer,
        text="0.00 kg",
        font=("Arial", 25, "bold"),
        text_color="#000000",
        width=140,
    )
    lbl_tot_peso.pack(side="left", padx=5, pady=5)

    lbl_tot_sacos = ctk.CTkLabel(
        footer,
        text="0",
        font=("Arial", 25, "bold"),
        text_color="#000000",
        width=80,
    )
    lbl_tot_sacos.pack(side="left", padx=5)

    ctk.CTkLabel(
        footer, text="total", font=("Arial", 22, "bold"), text_color="#000000"
    ).pack(side="right", padx=10)

    return {
        "header": lbl_header,
        "scroll": scroll,
        "lbl_peso": lbl_tot_peso,
        "lbl_sacos": lbl_tot_sacos,
    }

  def _bind_shortcuts(self):
    self.txt_padron.bind("<Return>", lambda e: self._buscar_socio())
    self.txt_sacos.bind("<Return>", lambda e: self._registrar_bajada())

  def _set_destino(self, destino: str):
    self.destino_actual = destino
    self.btn_f1.configure(
        fg_color="#0284C7" if destino == "MERCADO" else "#E2E8F0",
        text_color="#FFFFFF" if destino == "MERCADO" else "#000000",
    )
    self.btn_f2.configure(
        fg_color="#D97706" if destino == "FABRICA_LOCAL" else "#E2E8F0",
        text_color="#FFFFFF" if destino == "FABRICA_LOCAL" else "#000000",
    )
    self.btn_f3.configure(
        fg_color="#7C3AED" if destino == "FABRICA_PLANTA" else "#E2E8F0",
        text_color="#FFFFFF" if destino == "FABRICA_PLANTA" else "#000000",
    )

  def _buscar_socio(self):
    padron = self.txt_padron.get().strip()
    if not padron:
      return

    ag = self.agricultor_dao.buscar_por_padron(padron)
    if ag:
      self._limpiar_listas_pesaje_locales()
      self.agricultor_actual = ag
      nombre = f"{ag['nombres']} {ag.get('apellidos', '')}".strip()
      self.lbl_socio_nombre.configure(text=nombre, text_color="#000000")
      self.lbl_socio_doc.configure(
          text=ag.get("dni") or "NO REGISTRADO"
      )

      self.parcelas_map = {}
      opciones_combo = []

      for p in ag.get("parcelas", []):
        cod_int = p.get("codigo_interno") or "P01"
        nombre_p = p.get("nombre_parcela", "")
        sector_p = f" ({p['sector']})" if p.get("sector") else ""
        label_vis = f"{cod_int} - {nombre_p}{sector_p}".strip()

        opciones_combo.append(label_vis)
        self.parcelas_map[label_vis] = p

      if opciones_combo:
        self.cmb_parcela.configure(values=opciones_combo)
        self.cmb_parcela.set(opciones_combo[0])
      else:
        self.cmb_parcela.configure(values=["---"])
        self.cmb_parcela.set("---")

      # BUSCAR SI EXISTE BORRADOR EN PROCESO
      codigo_sesion = self.sesion_activa["codigo_sesion"]
      rec_proceso = self.recepcion_dao.obtener_recepcion_en_proceso(
          padron, codigo_sesion
      )

      if rec_proceso:
        self.recepcion_uuid_actual = rec_proceso["uuid"]
        for d in rec_proceso["detalles"]:
          item = {
              "uuid": d["uuid"],
              "peso": Decimal(str(d["peso_bruto"])),
              "sacos": d["numero_sacos"],
              "codigo_trazabilidad": d["codigo_trazabilidad"],
          }
          dest = d["destino"]
          self.pesadas[dest].append(item)
          self._crear_fila_ui(dest, item)

        self._actualizar_totales()
      else:
        self.recepcion_uuid_actual = str(uuid.uuid4())

      self.txt_sacos.focus_set()
      self.txt_sacos.select_range(0, "end")
    else:
      self.lbl_socio_nombre.configure(
          text="❌ NO ENCONTRADO", text_color="#EF4444"
      )
      self.lbl_socio_doc.configure(text="-")
      self.cmb_parcela.configure(values=["---"])
      self.cmb_parcela.set("---")
      self.agricultor_actual = None
      self.recepcion_uuid_actual = None
      self.parcelas_map = {}

  def _obtener_parcela_seleccionada(self) -> Optional[Dict]:
    val_combo = self.cmb_parcela.get()
    if hasattr(self, "parcelas_map") and val_combo in self.parcelas_map:
      return self.parcelas_map[val_combo]
    return None

  def _registrar_bajada(self):
    if (
        not self.agricultor_actual
        or not self.sesion_activa
        or not self.recepcion_uuid_actual
    ):
      return

    parcela_obj = self._obtener_parcela_seleccionada()
    if not parcela_obj:
      return

    try:
      peso_actual = Decimal(self.lbl_peso.cget("text"))
      num_sacos = int(self.txt_sacos.get().strip())
    except (ValueError, InvalidOperation):
      return

    if peso_actual <= Decimal("0.00") or num_sacos <= 0:
      return

    dest = (
        self.destino_actual
        if self.destino_actual in self.pesadas
        else "MERCADO"
    )

    if dest == "MERCADO":
      letra_destino = "M"
    elif dest == "FABRICA_LOCAL":
      letra_destino = "FL"
    else:
      letra_destino = "FP"

    codigo_sesion = self.sesion_activa["codigo_sesion"]
    padron = self.agricultor_actual["codigo_padron"]
    codigo_parcela_corta = parcela_obj.get("codigo_interno", "P01")
    codigo_parcela_oficial = parcela_obj.get("codigo_parcela")

    orden_bajada = (
        len(self.pesadas["MERCADO"])
        + len(self.pesadas["FABRICA_LOCAL"])
        + len(self.pesadas["FABRICA_PLANTA"])
        + 1
    )
    codigo_trazabilidad = f"{codigo_sesion}-{padron}-{codigo_parcela_corta}-{letra_destino}-{orden_bajada}"

    peso_contable = self._calcular_peso_contable(peso_actual)
    item_uuid = str(uuid.uuid4())
    fecha_actual = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    recepcion_data = {
        "uuid": self.recepcion_uuid_actual,
        "lote_acopio_id": None,
        "codigo_lote_origen": codigo_sesion,
        "codigo_padron": padron,
        "codigo_parcela": codigo_parcela_oficial,
        "producto": "MARACUYA",
        "fecha_pesaje": fecha_actual,
        "observaciones": None,
    }

    detalle_data = {
        "uuid": item_uuid,
        "codigo_trazabilidad": codigo_trazabilidad,
        "destino": dest,
        "numero_sacos": num_sacos,
        "peso_bruto": float(peso_actual),
        "peso_contable": float(peso_contable),
        "tara": 0.0,
        "peso_neto": float(peso_contable),
        "orden": orden_bajada,
        "fecha_pesaje": fecha_actual,
    }

    # 1. GUARDADO ATÓMICO EN SQLITE AL INSTANTE (Anti-caídas de luz)
    exito = self.recepcion_dao.registrar_bajada_individual(
        recepcion_data, detalle_data
    )

    if exito:
      # 2. IMPRESIÓN DE ETIQUETA SOLO SI EL CHECKBOX ESTÁ MARCADO (== 1)
      if (
          self.chk_imprimir_etiqueta.get() == 1
          and hasattr(self, "print_service")
          and self.print_service
      ):
        try:
          self.print_service.imprimir_etiquetas_trazabilidad([detalle_data])
        except Exception as e:
          print(f"⚠ [IMPRESORA]: Error imprimiendo etiqueta: {e}")

      # 3. DIBUJAR EN INTERFAZ
      item = {
          "uuid": item_uuid,
          "peso": peso_actual,
          "sacos": num_sacos,
          "codigo_trazabilidad": codigo_trazabilidad,
      }
      self.pesadas[dest].append(item)
      self._crear_fila_ui(dest, item)
      self._actualizar_totales()

    self.txt_sacos.focus_set()
    self.txt_sacos.select_range(0, "end")

  def _crear_fila_ui(self, dest: str, item: Dict):
    if dest == "MERCADO":
      panel = self.panel_tabla_mercado
    elif dest == "FABRICA_LOCAL":
      panel = self.panel_tabla_fab_local
    else:
      panel = self.panel_tabla_fab_planta

    scroll = panel["scroll"]

    row = ctk.CTkFrame(
        scroll,
        fg_color="#FFFFFF",
        height=40,
        corner_radius=0,
        border_width=1,
        border_color="#000000",
    )
    row.pack(fill="x", pady=1)

    ctk.CTkLabel(
        row,
        text=f"{item['peso']:.2f} kg",
        font=("Arial", 25, "bold"),
        text_color="#000000",
        width=140,
    ).pack(side="left", padx=5)
    ctk.CTkLabel(
        row,
        text=str(item["sacos"]),
        font=("Arial", 25, "bold"),
        text_color="#000000",
        width=80,
    ).pack(side="left", padx=5)

    item_id = item["uuid"]
    btn_del = ctk.CTkButton(
        row,
        text="x",
        font=("Arial", 20, "bold"),
        fg_color="#EF4444",
        hover_color="#DC2626",
        text_color="#FFFFFF",
        width=32,
        height=28,
        corner_radius=0,
        command=(
            lambda d=dest, uid=item_id: self._eliminar_bajada_por_uuid(d, uid)
        ),
    )
    btn_del.pack(side="right", padx=5)

    self.filas_ui[dest][item_id] = row

  def _eliminar_bajada_por_uuid(self, destino: str, item_id: str):
    if self.recepcion_uuid_actual:
      self.recepcion_dao.eliminar_bajada_individual(
          self.recepcion_uuid_actual, item_id
      )

    self.pesadas[destino] = [
        x for x in self.pesadas[destino] if x["uuid"] != item_id
    ]

    if item_id in self.filas_ui[destino]:
      widget = self.filas_ui[destino].pop(item_id)
      widget.destroy()

    self._actualizar_totales()

  def _eliminar_ultimo(self):
    dest = (
        self.destino_actual
        if self.destino_actual in self.pesadas
        else "MERCADO"
    )
    if self.pesadas[dest]:
      ultimo_item = self.pesadas[dest][-1]
      self._eliminar_bajada_por_uuid(dest, ultimo_item["uuid"])

  def _actualizar_totales(self):
    paneles = [
        ("MERCADO", self.panel_tabla_mercado),
        ("FABRICA_LOCAL", self.panel_tabla_fab_local),
        ("FABRICA_PLANTA", self.panel_tabla_fab_planta),
    ]
    for dest, panel in paneles:
      tot_peso = sum((x["peso"] for x in self.pesadas[dest]), Decimal("0.00"))
      tot_sacos = sum(x["sacos"] for x in self.pesadas[dest])
      panel["lbl_peso"].configure(text=f"{tot_peso:.2f} kg")
      panel["lbl_sacos"].configure(text=str(tot_sacos))

    tot_m = sum((x["peso"] for x in self.pesadas["MERCADO"]), Decimal("0.00"))
    tot_fl = sum(
        (x["peso"] for x in self.pesadas["FABRICA_LOCAL"]), Decimal("0.00")
    )
    tot_fp = sum(
        (x["peso"] for x in self.pesadas["FABRICA_PLANTA"]), Decimal("0.00")
    )

    self.lbl_tot_mercado.configure(text=f"{tot_m:.2f}kg")
    self.lbl_tot_fab_local.configure(text=f"{tot_fl:.2f}kg")
    self.lbl_tot_fab_planta.configure(text=f"{tot_fp:.2f}kg")

  def _calcular_peso_contable(self, peso_real: Decimal) -> Decimal:
    entero = int(peso_real)
    if (peso_real - entero) >= Decimal("0.90"):
      return Decimal(entero + 1)
    return Decimal(entero)

  def _pausar_pesaje(self):
    """Limpia la interfaz para atender a otro socio.

    El borrador queda seguro en SQLite.
    """
    self._reiniciar_pesaje()

  def _finalizar_pesaje_socio(self):
    """Genera el ticket definitivo, cambia el estado a COMPLETADO e imprime el comprobante."""
    if not self.agricultor_actual or not self.recepcion_uuid_actual:
      return

    bajadas_m = self.pesadas["MERCADO"]
    bajadas_fl = self.pesadas["FABRICA_LOCAL"]
    bajadas_fp = self.pesadas["FABRICA_PLANTA"]

    if not bajadas_m and not bajadas_fl and not bajadas_fp:
      return

    parcela_obj = self._obtener_parcela_seleccionada()
    if not parcela_obj:
      return

    codigo_ticket = self.recepcion_dao.generar_siguiente_codigo_ticket("RE01")
    fecha_cierre = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # 1. Cambiar estado a COMPLETADO en SQLite
    exito = self.recepcion_dao.finalizar_recepcion_en_proceso(
        self.recepcion_uuid_actual, codigo_ticket, fecha_cierre
    )

    if exito:
      # 2. Imprimir ticket de recibo consolidado
      if hasattr(self, "print_service") and self.print_service:
        try:
          todas_bajadas = []
          for dest in ["MERCADO", "FABRICA_LOCAL", "FABRICA_PLANTA"]:
            for b in self.pesadas[dest]:
              todas_bajadas.append({
                  "destino": dest,
                  "sacos": b["sacos"],
                  "peso": float(b["peso"]),
              })

          recibo_payload = {
              "codigo_ticket": codigo_ticket,
              "codigo_padron": self.agricultor_actual["codigo_padron"],
              "socio_nombre": (
                  f"{self.agricultor_actual['nombres']} {self.agricultor_actual.get('apellidos', '')}"
                  .strip()
              ),
              "documento": self.agricultor_actual.get("dni", "-"),
              "codigo_parcela": (
                  parcela_obj.get("codigo_interno")
                  or parcela_obj.get("codigo_parcela", "-")
              ),
              "sector": parcela_obj.get("sector", "-"),
              "fecha_pesaje": fecha_cierre,
              "total_sacos": sum(b["sacos"] for b in todas_bajadas),
              "peso_total": sum(b["peso"] for b in todas_bajadas),
              "bajadas": todas_bajadas,
          }
          self.print_service.imprimir_recibo_dec(recibo_payload)
        except Exception as e:
          print(f"⚠ [IMPRESORA]: Error imprimiendo ticket de recibo: {e}")

      print(f"✅ Recibo {codigo_ticket} finalizado correctamente.")

    self._reiniciar_pesaje()

  def _limpiar_listas_pesaje_locales(self):
    for dest in ["MERCADO", "FABRICA_LOCAL", "FABRICA_PLANTA"]:
      self.pesadas[dest].clear()
      for widget in self.filas_ui[dest].values():
        widget.destroy()
      self.filas_ui[dest].clear()
    self._actualizar_totales()

  def _reiniciar_pesaje(self):
    self._limpiar_listas_pesaje_locales()
    self.txt_padron.delete(0, "end")
    self.lbl_socio_nombre.configure(
        text="[ingrese un codigo padron]", text_color="#374151"
    )
    self.lbl_socio_doc.configure(text="-")
    self.cmb_parcela.configure(values=["---"])
    self.cmb_parcela.set("---")
    self.agricultor_actual = None
    self.recepcion_uuid_actual = None
    self.parcelas_map = {}
    self.txt_padron.focus_set()

  def _iniciar_lectura_balanza(self):
    if hasattr(self, "scale_service") and self.scale_service:
      peso = self.scale_service.get_weight()
      peso_texto = str(peso)
      if self.lbl_peso.cget("text") != peso_texto:
        self.lbl_peso.configure(text=peso_texto)

    self.after(30, self._iniciar_lectura_balanza)

  def _ejecutar_cierre_jornada(self):
    self.sesion_dao.cerrar_sesion_activa()
    if self.on_cerrar_sesion_cb:
      self.on_cerrar_sesion_cb()